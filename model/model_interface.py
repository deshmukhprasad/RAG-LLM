...
class MInterface(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        assert targets.size() == batch["tokens"].input_ids.size(), f"Size mismatch: targets size is {targets.size()}, but batch['tokens'].input_ids size is {batch['tokens'].input_ids.size()}"
    ...
    def load_llm(self, llm_path):
        print('Loading LLAMA')
        self.llama_tokenizer = AutoTokenizer.from_pretrained(llm_path, trust_remote_code=True)
