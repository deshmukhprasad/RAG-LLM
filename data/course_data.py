...
class CoursesData(data.Dataset):
    def __init__(self, data_dir=r'data/ref/courses',
                 stage=None,
                 cans_num=10,
                 sep=", ",
                 no_augment=True):
    ...
    def check_files(self):
        self.item_id2name = self.get_course_id2name()
        if self.stage == 'train':
    ...
    def get_course_id2name(self):
        course_id2name = dict()
        item_path = op.join(self.data_dir, 'u.item')
        with open(item_path, 'r', encoding="utf-8") as f:
            for l in f.readlines():
                ll = l.strip('\n').split('|')
                course_id2name[int(ll[0])] = ll[1]
        return course_id2name

    def session_data4frame(self, datapath, course_id2name):
        ...
        train_data['next'] = train_data['next'].apply(get_id_from_tuple)
